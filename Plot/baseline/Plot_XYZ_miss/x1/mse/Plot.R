...
mse_columns <- c(
  "mse_x1_fiml", 
  "mse_x1_pmm_default", "mse_x1_pmm_0.2","mse_x1_pmm_manual", 
  "mse_x1_norm_default", "mse_x1_norm_0.2","mse_x1_norm_manual",
  "mse_x1_cart_default", "mse_x1_cart_0.2","mse_x1_cart_manual"
)


# Create a mapping of the mse columns to shorter method names
...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr_x", mse_columns)]
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = mse, fill = num_aux)) +
      ...
      coord_cartesian(ylim=c(0, 0.065)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
    ...
    # ggsave(filename = paste0("xyz_mse_x1_n_", N_value, "_mry_", mr_y_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 17, height = 16)
    
    ggsave(filename = paste0("xyz_mse_x1_n_", N_value, "_mry_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16, dpi = 600)
  }
}
