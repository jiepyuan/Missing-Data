...
power_columns <- c(
  "power_x1_fiml", 
  "power_x1_pmm_default", "power_x1_pmm_0.2","power_x1_pmm_manual", 
  "power_x1_norm_default", "power_x1_norm_0.2","power_x1_norm_manual",
  "power_x1_cart_default", "power_x1_cart_0.2","power_x1_cart_manual"
)
...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    # ggsave(filename = paste0("xyz_power_x1_n_", N_value, "_mry_", mr_y_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 17, height = 16)
    
    ggsave(filename = paste0("xyz_power_x1_n_", N_value, "_mry_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16, dpi = 600)
  }
}
