...
power_columns <- c(
  "power_x2_fiml", 
  "power_x2_pmm_default", "power_x2_pmm_0.2","power_x2_pmm_manual", 
  "power_x2_norm_default", "power_x2_norm_0.2","power_x2_norm_manual",
  "power_x2_cart_default", "power_x2_cart_0.2","power_x2_cart_manual"
)

# Create a mapping of the power columns to shorter method names
...
names(imputation_methods) <- power_columns
...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr_x", power_columns)]
    
    # Reshape the data to long format
    df_long <- melt(df_plot, id.vars = c("missing_type", "num_aux", "b2_mr_x"),
                    variable.name = "imputation_method", value.name = "power")
    
    # Map the power column names to imputation method names
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = power, fill = num_aux)) +
      geom_col(position = position_dodge(width = 0.72), width = 0.58) +
      scale_fill_manual(
        values = c("#4477AA", "#EEAA33", "#333333"),
        name = "Number of Auxiliary Variables"
      ) +
      coord_cartesian(ylim=c(0, 1)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
    ...
    # ggsave(filename = paste0("xyz_power_x2_n_", N_value, "_mry_", mr_y_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 17, height = 16)
    
    ggsave(filename = paste0("xyz_power_x2_n_", N_value, "_mry_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16, dpi = 600)
  }
}
