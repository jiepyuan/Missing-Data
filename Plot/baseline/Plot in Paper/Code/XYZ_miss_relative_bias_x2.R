...
# Load data
load("XYZ_all_parameter.RData")
df <- subset(df_XY_all_parameter, 
             N ==200 & b2 %in% c(0.10, 0.50) & mr_x %in% c(0.10, 0.3) & mr_y==0.15)
...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr_x", relative_bias_columns)]
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = bias, fill = num_aux)) +
      ...
      coord_cartesian(ylim=c(-0.5, 0.2)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
    ...
    # ggsave(filename = paste0("type1_xyz_relative_bias_x2_n_", N_value, "_mry_", mr_y_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 17, height = 16)
    
    ggsave(filename = paste0("xyz_relative_bias_x2_n_", N_value, "_mry_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16, dpi = 600)
  }
}
