...
df <- subset(df_Y_all_parameter, 
             b2 %in% c(0.1) & mr %in% c(0.80) & missing_type %in% c("Y_MCAR") & N %in% c(30))
...
for (N_value in sort(unique(df$N))) {
  ...
  plot <- ggplot(df_long, aes(x = imputation_method, y = bias, fill = num_aux)) +
    ...
    theme(
      ...
      axis.text.x = element_text(size = 14, angle = 70, hjust = 1, vjust = 1),
  ...
  # ggsave(filename = paste0("y_relative_bias_x2_special", ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 15, height = 10)
  
  ggsave(filename = paste0("y_relative_bias_x2_special", ".png"), plot = plot, units = "in", width = 15, height = 10, dpi = 600)
}
