...
# Load data
load("df_XY_all_parameter_diffuse.RData")
...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    # Map the bias column names to imputation method names
    ...
    # ggsave(filename = paste0("xyz_relative_bias_x2_n_", N_value, "_mry_", mr_y_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 17, height = 16)
