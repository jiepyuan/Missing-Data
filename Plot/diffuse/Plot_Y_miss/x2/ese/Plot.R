...
for (N_value in sort(unique(df$N))) {
  ...
  df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr", ese_columns)]
  ...
  # ggsave(filename = paste0("y_ese_x2_n_", N_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 15, height = 10)
  
  ggsave(filename = paste0("y_ese_x2_n_", N_value, ".png"), plot = plot, units = "in", width = 15, height = 10, dpi = 600)
}
