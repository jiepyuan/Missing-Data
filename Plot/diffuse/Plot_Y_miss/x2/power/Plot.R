...
for (N_value in sort(unique(df$N))) {
  ...
  # ggsave(filename = paste0("y_power_x2_n_", N_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 15, height = 10)
  
  ggsave(filename = paste0("y_power_x2_n_", N_value, ".png"), plot = plot, units = "in", width = 15, height = 10, dpi = 600)
}
