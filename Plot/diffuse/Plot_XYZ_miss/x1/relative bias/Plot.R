...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = bias, fill = num_aux)) +
      ...
      coord_cartesian(ylim=c(-0.9, 0.2)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
    ...
    # ggsave(filename = paste0("xyz_relative_bias_x1_n_", N_value, "_mry_", mr_y_value, ".pdf"), plot = plot, device = cairo_pdf, units = "in", width = 17, height = 16)
    
    ggsave(filename = paste0("xyz_relative_bias_x1_n_", N_value, "_mry_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16, dpi = 600)
  }
}
