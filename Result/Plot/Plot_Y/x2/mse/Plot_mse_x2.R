...
for (N_value in sort(unique(df$N))) {
  ...
  plot <- ggplot(df_long, aes(x = imputation_method, y = mse, color = num_aux, group = num_aux)) +
    geom_line() +
    geom_point() +
    coord_cartesian(ylim=c(0, 0.07)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
  ...
  ggsave(filename = paste0("mse_x2_n_", N_value, ".png"), plot = plot, units = "in", width = 15, height = 10)
}
