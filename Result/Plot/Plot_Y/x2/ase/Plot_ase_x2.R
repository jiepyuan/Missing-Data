...
ase_columns <- c(
  "ase_x2_fiml", 
  "ase_x2_pmm_default", "ase_x2_pmm_0.2","ase_x2_pmm_manual", 
  "ase_x2_norm_default", "ase_x2_norm_0.2", "ase_x2_norm_manual",
  "ase_x2_cart_default", "ase_x2_cart_0.2", "ase_x2_cart_manual"
)

# Create a mapping of the ase columns to shorter method names
...
for (N_value in sort(unique(df$N))) {
  ...
  df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr", ase_columns)]
  
  # Reshape the data to long format
  df_long <- melt(df_plot, id.vars = c("missing_type", "num_aux", "b2_mr"),
                  variable.name = "imputation_method", value.name = "ase")
  
  # Map the ase column names to imputation method names
  ...
  plot <- ggplot(df_long, aes(x = imputation_method, y = ase, color = num_aux, group = num_aux)) +
    geom_line() +
    geom_point() +
  ...
  ggsave(filename = paste0("ase_x2_n_", N_value, ".png"), plot = plot, units = "in", width = 15, height = 10)
}
