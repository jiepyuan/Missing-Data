...
for (N_value in sort(unique(df$N))) {
  ...
  plot <- ggplot(df_long, aes(x = imputation_method, y = power, color = num_aux, group = num_aux)) +
    geom_line() +
    geom_point() +
  ...
  ggsave(filename = paste0("power_x1_n_", N_value, ".png"), plot = plot, units = "in", width = 15, height = 10)
}
