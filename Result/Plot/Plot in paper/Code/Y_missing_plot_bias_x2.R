...
bias_columns <- c(
  "bias_x2_fiml", 
  "bias_x2_pmm_default", "bias_x2_pmm_0.2", "bias_x2_pmm_manual",
  "bias_x2_norm_default", "bias_x2_norm_0.2", "bias_x2_norm_manual",  # moved norm before cart
  "bias_x2_cart_default", "bias_x2_cart_0.2", "bias_x2_cart_manual"
)

# Create a mapping of the bias columns to shorter method names
...
names(imputation_methods) <- bias_columns
...
for (N_value in sort(unique(df$N))) {
  ...
  df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr", bias_columns)]
  ...
  plot <- ggplot(df_long, aes(x = imputation_method, y = bias, color = num_aux, group = num_aux)) +
    geom_line() +
    geom_point() +
    coord_cartesian(ylim=c(-0.6, 0.2)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
  ...
  ggsave(filename = paste0("y_bias_x2_n_", N_value, ".png"), plot = plot, units = "in", width = 15, height = 10)
}
