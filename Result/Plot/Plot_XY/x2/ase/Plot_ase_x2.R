...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr_x", ase_columns)]
    ...
    ggsave(filename = paste0("ase_x2_n_", N_value, "_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16)
  }
}
