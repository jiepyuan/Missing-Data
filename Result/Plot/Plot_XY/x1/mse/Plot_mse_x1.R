...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = mse, color = num_aux, group = num_aux)) +
      geom_line() +
      geom_point() +
    ...
    ggsave(filename = paste0("mse_x1_n_", N_value, "_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16)
  }
}