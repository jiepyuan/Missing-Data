...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    df_long <- melt(df_plot, id.vars = c("missing_type", "num_aux", "b2_mr_x"),
                    variable.name = "imputation_method", value.name = "relative_bias")
    
    # Map the relative_bias column names to imputation method names
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = relative_bias, color = num_aux, group = num_aux)) +
      geom_line() +
      geom_point() +
      coord_cartesian(ylim=c(-0.3, 0.3)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
    ...
    ggsave(filename = paste0("relative_bias_x1_n_", N_value, "_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16)
  }
}