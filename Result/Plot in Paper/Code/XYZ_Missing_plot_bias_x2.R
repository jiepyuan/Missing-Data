...
for (N_value in sort(unique(df$N))) {
  for (mr_y_value in sort(unique(df$mr_y))){
    ...
    df_plot <- df_N[, c("missing_type", "num_aux", "b2_mr_x", bias_columns)]
    ...
    plot <- ggplot(df_long, aes(x = imputation_method, y = bias, color = num_aux, group = num_aux)) +
      geom_line() +
      geom_point() +
      coord_cartesian(ylim=c(-0.2, 0.1)) +   # we need to use the coord_cartesian because it will prevent R to remove the value this is out of bound
    ...
    ggsave(filename = paste0("xyz_bias_x2_n_", N_value, "_mry_", mr_y_value, ".png"), plot = plot, units = "in", width = 17, height = 16)
  }
}